...
export async function scrapeConcertsOnTheSlope() {
  ...
  try {
    ...
    const result = await extractEventsFromPage(
      page,
      "Extract all visible concerts on this 2025-2026 season page. For each event, get eventName, eventDate with YEAR, eventTime (format like 3:00 PM), eventDescription (if available), eventUrl (full href of the tickets or title link; if relative, prepend https://www.concertsontheslope.org; do not return internal IDs), and set eventLocation to 'St. John's Episcopal Church' for all events. If description is not visible, return an empty string for eventDescription.",
      SeasonEventSchema,
