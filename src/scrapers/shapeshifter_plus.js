  ...
  try {
    ...
    while (pageCount < maxPages) {
      ...
      const result = await extractEventsFromPage(
        page,
        "Extract all visible events from the page. For each event: 1) get the event title text (as eventName); 2) get the event date (as eventDate, include the full year, e.g., 'Thursday, October 30, 2025'); 3) get the event time if available (as eventTime, e.g., '7:00 pm') — if no time is present, return an empty string for eventTime; 4) get the event description if available (as eventDescription) — if no description is present, return an empty string for eventDescription; 5) set eventLocation to 'Shapeshifter Plus' for all events; 6) click or inspect the LINK on the EVENT NAME/TITLE and return the anchor href to the event details page (as eventUrl). Returning a relative URL is acceptable. Do not return IDs like '0-459'; return the actual href from the event title link.",
        LenientEventSchema,
