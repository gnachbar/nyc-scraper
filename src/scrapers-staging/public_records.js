...
export async function scrapePublicRecords() {
  ...
  try {
    ...
    const result = await extractEventsFromPage(
      page,
      "Extract all visible events from the Public Records venue page. For each event, get: (1) eventName: the full event title; (2) eventDate: RETURN IN EXACT FORMAT 'Monday, October 27, 2025' using full weekday and full month names with a comma after weekday and after day, and include the 4-digit year; DO NOT use abbreviated weekday/month (e.g., NOT 'Thu, Oct 30, 2025'); (3) eventTime: the time if visible (e.g., '7:00 PM'); if no time is shown, return an empty string; (4) eventLocation: set to 'Public Records' for all events; (5) eventUrl: the full absolute URL to the event details page by clicking the event title or tickets link and capturing the href. Ensure eventDate strictly matches 'Monday, October 27, 2025'.",
      StandardEventSchema,
