...
- `src/scripts/migrate_add_category_recurring.py` - DB migration adding category/recurring (and optional first_seen_at if needed).
...
- [ ] 3.0 Implement recurring events flag (same-title across dates) and sidebar toggle
  - [ ] 3.1 Migration: add `is_recurring` (bool), `recurrence_key` (text) to events table
  - [ ] 3.2 Define `recurrence_key = normalize(title)` function (lowercase, trim, collapse whitespace, strip punctuation)
  - [ ] 3.3 Post-import job: group by `(venue, recurrence_key)` and set `is_recurring = true` when count of distinct dates ≥ 2
  - [ ] 3.4 Backfill existing events to set flags accordingly
  - [ ] 3.5 UI: add sidebar toggle "Recurring" (on/off) in `templates/index.html` and wire in `static/js/main.js`
  - [ ] 3.6 Backend: filter param handling in `src/web/app.py` for recurring toggle
  - [ ] 3.7 Tests: DB grouping correctness; UI filter integration
